...
int main(int _argc, char **_argv) {
    ...
    do {
        ...
        if (strcmp(data, "todo-print-all") == 0) {
            ...
            td++;
        } else if (strcmp(data, "todo-status") == 0) {
            data = strtok(NULL, "#");
            id = atoi(data);
            data = strtok(NULL, "#");
            strcpy(status, data);
            ...
            if (id > 0 && id <= td) {
                strcpy(activities[id - 1].status, status);
                file = fopen("storage/todo-repository.txt", "w+");
                if (file == NULL) {
                    printf("Tidak dapat membuka file.\n");
                } else {
                    for (int i = 0; i < td; i++) {
                        fprintf(file, "%d|%s|%s|%s\n", activities[i].id, activities[i].time, activities[i].description, activities[i].status);
                    }
                }
                fclose(file);
            }
